 ...
 */
  ...
  */
$nzshpcrt_gateways[1] = array(
	'name' => 'PayPal Payments Standard 2.0',
	'api_version' => 2.0,
	'image' => WPSC_URL . '/images/paypal.gif',
	'class_name' => 'wpsc_merchant_paypal_standard_gd',
	...
	'internalname' => 'wpsc_merchant_paypal_standard_gd',

	// All array members below here are legacy, and use the code in paypal_multiple.php
	'form' => 'form_paypal_multiple_gd',
	'submit_function' => 'submit_paypal_multiple_gd',
	'payment_type' => 'paypal',
...
*/
class wpsc_merchant_paypal_standard_gd extends wpsc_merchant {
	...
	*/
	...
	function get_paypal_currency_code() {
		if ( empty( $this->paypal_currency_code ) ) {
			...
			if ( ! in_array( $this->paypal_currency_code, $wpsc_gateways['wpsc_merchant_paypal_standard_gd']['supported_currencies']['currency_list'] ) )
	...
	*/
	function _construct_value_array($aggregate = false) {
		...
		if (get_option('paypal_ipn') == 1) {
			...
			$notify_url = add_query_arg('gateway', 'wpsc_merchant_paypal_standard_gd', $notify_url);
...
/**
 * form_paypal_multiple_gd function.
 *
 * Use this for now, but it will eventually be replaced with a better form API for gateways
 * @access public
 * @return void
 */
function form_paypal_multiple_gd() {
  ...
  <tr>
   ...
   <tr>
	...
	if(($current_currency == '') && in_array($store_currency_data['code'], $wpsc_gateways['wpsc_merchant_paypal_standard_gd']['supported_currencies']['currency_list'])) {
  ...
  <tr>
		...
		$paypal_currency_list = array_map( 'esc_sql', $wpsc_gateways['wpsc_merchant_paypal_standard_gd']['supported_currencies']['currency_list'] );

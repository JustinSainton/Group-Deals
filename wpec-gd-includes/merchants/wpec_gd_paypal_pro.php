...
$nzshpcrt_gateways[2] = array(
	'name'                   => 'PayPal Pro 2.0',
	'api_version'            => 2.0,
	'class_name'             => 'wpsc_merchant_paypal_pro_gd',
	...
	'form'                   => 'form_paypal_pro_gd',
	'submit_function'        => 'submit_paypal_pro_gd',
	'internalname'           => 'wpsc_merchant_paypal_pro_gd', // this may be legacy, not yet decided
 ...
 */
class wpsc_merchant_paypal_pro_gd extends wpsc_merchant {
	...
	function get_paypal_currency_code() {
		if ( empty( $this->paypal_currency_code ) ) {
			...
			if ( ! in_array( $this->paypal_currency_code, $wpsc_gateways['wpsc_merchant_paypal_pro_gd']['supported_currencies']['currency_list'] ) )
	...
	function construct_value_array() {
		...
		$data['NOTIFYURL']       = add_query_arg( 'gateway', 'wpsc_merchant_paypal_pro_gd', $this->cart_data['notification_url'] );
...
function form_paypal_pro_gd() {
	...
	if(($current_currency == '') && in_array($store_currency_code, $wpsc_gateways['wpsc_merchant_paypal_pro_gd']['supported_currencies']['currency_list'])) {
	...
	if($current_currency != $store_currency_code) {
		...
		if (!isset($wpsc_gateways['wpsc_merchant_paypal_pro_gd']['supported_currencies']['currency_list']))
			$wpsc_gateways['wpsc_merchant_paypal_pro_gd']['supported_currencies']['currency_list'] = array();

		$paypal_currency_list = array_map( 'esc_sql', $wpsc_gateways['wpsc_merchant_paypal_pro_gd']['supported_currencies']['currency_list'] );
...
if ( in_array( 'wpsc_merchant_paypal_pro_gd', (array)get_option( 'custom_gateway_options' ) ) ) {
	...
	$gateway_checkout_form_fields[$nzshpcrt_gateways[2]['internalname']] = "
	<tr>
